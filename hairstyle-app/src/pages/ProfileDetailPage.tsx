      ...
      <div className="pb-10" style={{ background: 'var(--bg)', minHeight: '100svh' }}>
        ...
        <div className="px-4 pt-4">
          <div className="p-4 rounded-2xl mb-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-1)', fontFamily: 'Manrope' }}>Mes favoris</h1>
            <p style={{ fontSize: 12, color: 'var(--text-2)', fontFamily: 'Manrope', marginTop: 6 }}>{totalFavorites} element{totalFavorites > 1 ? 's' : ''} sauvegarde{totalFavorites > 1 ? 's' : ''} pour reserver plus vite.</p>
          </div>
          {totalFavorites === 0 && <div className="flex flex-col items-center py-14 gap-3"><div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: 'var(--surface)', fontSize: 26 }}>♡</div><p style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'Manrope' }}>Aucun favori pour le moment</p><p style={{ fontSize: 12, color: 'var(--text-3)', fontFamily: 'Manrope', textAlign: 'center', maxWidth: 240 }}>Ajoutez une coiffeuse, un service ou un style pour le retrouver ici.</p><button onClick={() => navigate('/hairstyles')} className="px-4 py-3 rounded-xl active-scale" style={{ background: 'var(--gold)', color: '#FFFFFF', fontSize: 13, fontWeight: 700, fontFamily: 'Manrope' }}>Explorer les styles</button></div>}
          {savedStylists.length > 0 && <div className="mb-5"><p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', fontFamily: 'Manrope', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8, paddingLeft: 4 }}>Coiffeuses</p>{savedStylists.map(stylist => <div key={stylist.id} className="mb-3"><HairstylistCard stylist={stylist} /><button type="button" onClick={() => setFavorites(current => ({ ...current, hairstylistIds: toggleId(current.hairstylistIds, stylist.id) }))} className="w-full -mt-1 py-2 rounded-xl active-scale" style={{ background: 'rgba(239,68,68,0.07)', color: '#DC2626', border: '1px solid rgba(239,68,68,0.18)', fontSize: 12, fontWeight: 600, fontFamily: 'Manrope' }}>Retirer des favoris</button></div>)}</div>}
          {savedServices.length > 0 && <div className="mb-5"><p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', fontFamily: 'Manrope', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8, paddingLeft: 4 }}>Services</p><div className="flex flex-col gap-3">{savedServices.map(service => { const stylist = hairstylists.find(item => item.services.includes(service.id)); return <div key={service.id} className="flex items-center gap-3 p-3.5 rounded-2xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}><img src={service.image} alt={service.name} className="w-14 h-14 rounded-full object-cover flex-shrink-0" /><div className="flex-1 min-w-0"><p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'Manrope' }}>{service.name}</p><p style={{ fontSize: 11, color: 'var(--text-3)', fontFamily: 'Manrope' }}>{service.duration} · {service.price}€</p></div><button onClick={() => navigate(stylist ? `/booking?serviceId=${service.id}&stylistId=${stylist.id}` : '/hairstylists')} className="px-3 py-2 rounded-xl active-scale" style={{ background: 'var(--cta-gradient)', color: '#FFFFFF', fontSize: 12, fontWeight: 700, fontFamily: 'Manrope' }}>Reserver</button></div> })}</div></div>}
          {savedStyles.length > 0 && <div className="mb-5"><p style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', fontFamily: 'Manrope', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8, paddingLeft: 4 }}>Styles</p><div className="grid grid-cols-2 gap-3">{savedStyles.map(style => <div key={style.id} className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}><button type="button" onClick={() => navigate(`/hairstylists?category=${style.category}`)} className="w-full text-left active-scale"><div style={{ height: 128 }}><img src={style.image} alt={style.name} className="w-full h-full object-cover" /></div><div className="p-2.5"><p className="truncate" style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'Manrope' }}>{style.name}</p><p style={{ fontSize: 11, fontWeight: 700, color: 'var(--gold)', fontFamily: 'Manrope' }}>des {style.startingPrice}€</p></div></button><button type="button" onClick={() => setFavorites(current => ({ ...current, hairstyleIds: toggleId(current.hairstyleIds, style.id) }))} className="w-full py-2 active-scale" style={{ background: 'rgba(239,68,68,0.07)', color: '#DC2626', borderTop: '1px solid rgba(239,68,68,0.18)', fontSize: 11, fontWeight: 600, fontFamily: 'Manrope' }}>Retirer</button></div>)}</div></div>}
        </div>
      </div>
    )
  }

  if (!config) return <div className="pb-10" style={{ background: 'var(--bg)', minHeight: '100svh' }}><Header title="Profil" showBack /><div className="px-4 pt-12 flex flex-col items-center gap-3"><p style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'Manrope' }}>Section introuvable</p><button onClick={() => navigate('/profile')} className="px-4 py-3 rounded-xl active-scale" style={{ background: 'var(--gold)', color: '#FFFFFF', fontSize: 13, fontWeight: 700, fontFamily: 'Manrope' }}>Retour au profil</button></div></div>

  return (
    <div className="pb-10" style={{ background: 'var(--bg)', minHeight: '100svh' }}>
      <Header title={config.title} showBack />
      <div className="px-4 pt-4">
        <div className="p-5 rounded-2xl mb-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}><div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4" style={{ background: 'var(--gold-light)', border: '1px solid var(--gold-border)', fontSize: 12, color: 'var(--gold)', fontWeight: 800 }}>{config.icon}</div><h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-1)', fontFamily: 'Manrope', lineHeight: 1.2 }}>{config.title}</h1><p style={{ fontSize: 12, lineHeight: 1.6, color: 'var(--text-2)', fontFamily: 'Manrope', marginTop: 8 }}>{config.intro}</p></div>
        {section === 'notification-settings' && <div className="flex flex-col gap-3 mb-4">{[{ key: 'reminders', label: 'Rappels rendez-vous' }, { key: 'offers', label: 'Offres et nouveautés' }, { key: 'sms', label: 'SMS de confirmation' }].map(item => { const key = item.key as keyof typeof toggles; return <button key={item.key} onClick={() => setToggles(current => ({ ...current, [key]: !current[key] }))} className="w-full flex items-center justify-between p-4 rounded-2xl active-scale" style={{ background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}><span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'Manrope' }}>{item.label}</span><span className="w-11 h-6 rounded-full flex items-center p-0.5" style={{ background: toggles[key] ? 'var(--gold)' : 'var(--surface-2)', transition: 'background 0.18s ease' }}><span className="w-5 h-5 rounded-full" style={{ background: '#fff', transform: toggles[key] ? 'translateX(20px)' : 'translateX(0)', transition: 'transform 0.18s ease' }} /></span></button> })}</div>}
        <div className="rounded-2xl overflow-hidden mb-4" style={{ border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>{detailRows.map((row, index) => { const tone = row.tone ? toneColors[row.tone] : undefined; return <div key={row.label} className="flex items-center justify-between gap-3 p-4" style={{ background: '#fff', borderBottom: index < detailRows.length - 1 ? '1px solid var(--border)' : 'none' }}><div><p style={{ fontSize: 11, color: 'var(--text-3)', fontFamily: 'Manrope', marginBottom: 2 }}>{row.label}</p><p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', fontFamily: 'Manrope' }}>{row.value}</p></div>{tone && <span style={{ color: tone.color, background: tone.background, border: `1px solid ${tone.border}`, borderRadius: 99, padding: '3px 9px', fontSize: 10, fontWeight: 700, fontFamily: 'Manrope' }}>Actif</span>}</div> })}</div>
        {feedback && <div className="mb-3 p-3 rounded-xl" style={{ background: 'var(--gold-light)', border: '1px solid var(--gold-border)', color: 'var(--gold)', fontSize: 12, fontWeight: 700, fontFamily: 'Manrope' }}>{feedback}</div>}
        <button onClick={() => showFeedback(section === 'referral' ? `Code ${profile.referralCode} copié.` : 'Action enregistrée pour la démo.')} className="w-full py-4 rounded-2xl active-scale" style={{ background: 'var(--cta-gradient)', color: '#FFFFFF', fontSize: 14, fontWeight: 700, fontFamily: 'Manrope', boxShadow: '0 4px 16px rgba(196,69,115,0.28)' }}>{config.primaryLabel}</button>
      </div>
    </div>
  )
}
